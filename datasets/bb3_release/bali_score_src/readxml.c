...
static void do_element(void)
{
    ...
    if (strcmp(Element, "sequence") == 0) {
        ...
        if (Nftable[Ftype] >= MAXFT)
            ...
            alloc_ft_entry(&Maln.ft[Fseq + Nseqs].data[Ftype][Nftable[Ftype]]);
        if (Nftable[Ftype] < MAXFT)
            strcpy(Maln.ft[Fseq + Nseqs].data[Ftype][Nftable[Ftype]].type, Content);
    } else if (strcmp(Element, "fstart") == 0) {
        if (Nftable[Ftype] < MAXFT)
            Maln.ft[Fseq + Nseqs].data[Ftype][Nftable[Ftype]].start = atoi(Content) - 1;
    } else if (strcmp(Element, "fstop") == 0) {
        if (Nftable[Ftype] < MAXFT)
            Maln.ft[Fseq + Nseqs].data[Ftype][Nftable[Ftype]].end = atoi(Content) - 1;
    } else if (strcmp(Element, "fcolor") == 0) {
        if (Nftable[Ftype] < MAXFT)
            Maln.ft[Fseq + Nseqs].data[Ftype][Nftable[Ftype]].color = atoi(Content);
    } else if (strcmp(Element, "fscore") == 0) {
        if (Nftable[Ftype] < MAXFT)
            Maln.ft[Fseq + Nseqs].data[Ftype][Nftable[Ftype]].score = atof(Content);
    } else if (strcmp(Element, "fnote") == 0) {
        if (Nftable[Ftype] < MAXFT)
            if (Content != NULL)
                strcpy(Maln.ft[Fseq + Nseqs].data[Ftype][Nftable[Ftype]].name, Content);
        ...
        for (i = 0, j = 0; i < l; i++)
            if (!isspace(Content[i]))
                Maln.seqs[Fseq + Nseqs].data[j++] = Content[i];
    ...
    if (Element != NULL)
        Element = ckfree(Element);
    if (Content != NULL)
        Content = ckfree(Content);
}
