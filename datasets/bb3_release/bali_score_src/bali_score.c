...
int main(int argc, char** argv)
{
    ...
    for (i = 0; i < ref_aln.nseqs; i++)
        if (refmaxlen < ref_aln.seqs[i].len)
            refmaxlen = ref_aln.seqs[i].len;
    ...
    for (i = 0; i < test_aln.nseqs; i++)
        if (maxlen < test_aln.seqs[i].len)
            maxlen = test_aln.seqs[i].len;
...
if gaps, =nseqs otherwise */
    ...
    for (i = 0; i < maxlen; i++) {
        if (gap[i] >= cutoff)
            ...
            refseq_col[i] = ref_aln->nseqs - gap[i];
    }
    ckfree(gap);
}
    ...
    for (seq = 0; seq < test_aln->nseqs; seq++) {
        ...
        for (i = 0; i < seqlength; i++) {
            if (test_aln->seqs[seq].data[i] == '-') {
                ...
                if (refseq_col[ix - 1] > 0 && ref_aln->seqs[seq_xref[seq]].data[ix - 1] != '-')
                    seq_code[seq_xref[seq]][i] = ix;
    ...
    for (seqno = 0; seqno < nseqs; seqno++) {
        ...
        for (i = 0;; i++) {
            if (fgets(line, MAXLINE + 1, fin) == NULL)
                return mult_aln; /* read the title*/
            if (line[0] == '/' && line[1] == '/')
                break;
        }
