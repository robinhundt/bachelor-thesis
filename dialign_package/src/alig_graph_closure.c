    ...
    for (n = 1; n <= clos->nbrAligSets; n++) {
        ...
        for (x = 0; x < clos->seqNbr; x++)
            if (clos->aligSet[n].pos[x] > 0) {
                ...
                for (i = pos_n + 1;
                     i <= clos->seq[x].longueur && clos->seq[x].aligSetNbr[i] == 0; i++)
...
void alloc_closure(CLOSURE* clos)
{
    ...
    clos->predFrontier = (int**)callouer_mat(
        sizeof(int), clos->maxLong + 2, clos->seqNbr + 1); /* sera re'alloue' */
    clos->succFrontier = (int**)callouer_mat(
        sizeof(int), clos->maxLong + 2, clos->seqNbr + 1); /* sera re'alloue' */

    clos->aligSet = (positionSet*)allouer(
        (clos->maxLong + 2) * sizeof(positionSet)); /* sera re'alloue' */
    ...
    for (x = 0; x < clos->seqNbr; x++) {
        ...
        clos->seq[x].predAligSetPos = (int*)allouer((clos->seq[x].longueur + 2) * sizeof(int));
        clos->seq[x].succAligSetPos = (int*)allouer((clos->seq[x].longueur + 2) * sizeof(int));
    }
...
void realloc_closure(CLOSURE* clos)
{
    ...
    if (clos->nbrAligSets > clos->oldNbrAligSets) {
        clos->predFrontier = (int**)recallouer_mat(
            (void**)clos->predFrontier, sizeof(int), clos->oldNbrAligSets + 2,
            clos->nbrAligSets + 2, clos->seqNbr + 1);
        clos->succFrontier = (int**)recallouer_mat(
            (void**)clos->succFrontier, sizeof(int), clos->oldNbrAligSets + 2,
            clos->nbrAligSets + 2, clos->seqNbr + 1);
        clos->aligSet = (positionSet*)reallouer(
            clos->aligSet, (clos->nbrAligSets + 2) * sizeof(positionSet));
...
CLOSURE*
newAligGraphClosure(int nbreseq, int* longseq, int nbreancr, int** ancrages)
{
...
int addAlignedPositions(CLOSURE* clos, int seq1, int i, int seq2, int j)
{
    ...
    // retrieve eq class numbers
    // n is probably next class
    // ng is next class gauche (pred)
    // nd is is next class droite (succ)
    n1 = ng1 = nd1 = clos->seq[seq1].aligSetNbr[i];
    n2 = ng2 = nd2 = clos->seq[seq2].aligSetNbr[j];

    // if i or j are not aligned (eq class 0) or part of different eq classes
    if (n1 == 0 || n2 == 0 || n1 != n2) {
        // if (i, s1) is not aligned
        if (ng1 == 0) {
            // k is next aligned position < j
            k = clos->seq[seq1].predAligSetPos[i];
            if (k > 0)
                // when there is a position k in s1 that is aligned (k > 0)
                // store eq class number in ng1
                ng1 = clos->seq[seq1].aligSetNbr[k];

            // do the same for succ and nd1
            k = clos->seq[seq1].succAligSetPos[i];
            if (k > 0)
                nd1 = clos->seq[seq1].aligSetNbr[k];
        }
        // if (j, s2) is not aligned
        // eq to if block above, store next pred and succ class in ng2 and nd2 resp.
        if (ng2 == 0) {
        ...
        if (ng1 == 0) // no pred next class
            for (x = 0; x < clos->seqNbr; x++)
                // since there is no next class, set gauche to 0, meaning
                // s1 is not aligned to s2 and the pred frontiers have the default
                // val of 0
                clos->gauche1[x] = 0;
        else
            // there is a next class so gauche is initialized to the pred frontiers of
            // that class
            for (x = 0; x < clos->seqNbr; x++)
        ...
        if (nd1 == 0)
            for (x = 0; x < clos->seqNbr; x++)
                // the default val for droite (succ tmp buffer prob) is the length
                // of the seq + 1
        ...
        // align i and j by setting the respective frontiers
        clos->gauche1[seq1] = clos->droite1[seq1] = i;
        clos->gauche2[seq2] = clos->droite2[seq2] = j;

        // nn seems to be the current handled eq class
        // not sure why it's simply an incremented counter. Might become clear later
        nn = clos->nbrAligSets + 1;

        for (x = 0; x < clos->seqNbr; x++) {
            // this inializes alignment info with 0, meaning eq class nn is aligned with no seq
            clos->aligSet[nn].pos[x] = 0;
            // check whether s1 or s2 are aligned with x and set aligSet[nn].pos[x] accordingly
            ...
            // only if aligSet[x].pos[x] is still 0, update pred and succ by doing max or min resp
            // in the other case the frontiers just coincide with aligSet[nn].pos[x] because nn is
            // directly aligned to x 
        ...
        // set frontiers of eq class to seqs to aligned pos i and j
        ...
        // update eq class ids in seq[x].aligSetNbr[k] for every site in new eq class
        // which is potentially a merge of two others
        ...
        for (x = 0; x < clos->seqNbr; x++)
            if (clos->droite1[x] != clos->droite2[x]) /* => la front. clos->gauche peut changer */
                for (y = 0; y < clos->seqNbr; y++) {
                    // default is 0
                    // stores the right most position in sequence x that is part of
                    // an eq class for which the pred frontier to y is less than
                    // that of the currently workend on class
                    clos->pos_[x][y] = 0;
        ...
        if (n1 == 0) {
            ...
            if (k > 0)
                clos->seq[seq1].succAligSetPos[k] = i;
            for (k = i + 1;
                 k <= clos->seq[seq1].longueur && clos->seq[seq1].aligSetNbr[k] == 0;
                 k++)
        ...
        if (n2 == 0) {
            ...
            if (k > 0)
                clos->seq[seq2].succAligSetPos[k] = j;
            for (k = j + 1;
                 k <= clos->seq[seq2].longueur && clos->seq[seq2].aligSetNbr[k] == 0;
                 k++)
...
{
    return (x == y && i == j) || (clos->seq[x].aligSetNbr[i] != 0 && clos->seq[x].aligSetNbr[i] == clos->seq[y].aligSetNbr[j]);
}
