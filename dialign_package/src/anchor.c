...
int multi_anc_read(char* file_name)
{
    ...
    while (fgets(line, MLINE, fp) != NULL) {
        if (word_count(line) == 6) {
            sscanf(line, "%d %d %d %d %d %f ", &seq1, &seq2, &beg1, &beg2, &len,
                &wgt);
            ...
            current_frg->next = (struct multi_frag*)calloc(1, sizeof(struct multi_frag));
