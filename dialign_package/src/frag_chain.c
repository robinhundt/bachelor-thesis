...
*/
    ...
    if ((prec_vec = (struct pair_frag**)calloc(
             (seqlen[n2] + 3), sizeof(struct pair_frag*)))
        == NULL) {
        printf("problems with memory allocation for prec_vec!  \n \n");
        exit(1);
    }

    if ((diap = (struct pair_frag**)calloc((seqlen[n1] + 3),
    ...
    if ((diap[0] = (struct pair_frag*)calloc(1, sizeof(struct pair_frag))) == NULL) {
...
*/
            ...
            for (j = lb_int[i]; j <= limit; j++) {
                ...
                if (wgt_type % 2) {
                    if (strict)
                        start_dna = ((seq[n1][i] == seq[n2][j]) && (seq[n1][i] == 'A' || seq[n1][i] == 'C' || seq[n1][i] == 'T' || seq[n1][i] == 'G' || seq[n1][i] == 'U'));
                    ...
                    if (dna_speed)
                        if ((i < seqlen[n1]) && (j < limit)) {
                            if (strict)
                                start_dna1 = ((seq[n1][i + 1] == seq[n2][j + 1]) && (seq[n1][i + 1] == 'A' || seq[n1][i + 1] == 'C' || seq[n1][i + 1] == 'T' || seq[n1][i + 1] == 'G' || seq[n1][i + 1] == 'U'));
...
*/
                    ...
                    kmaxloc = mini3(lmax_real, seqlen[n1] - i + 1, seqlen[n2] - j + 1);

                    if (motifs) {
                        for (k = 1; k <= kmaxloc; k++)
                            if ((mot_pos[n1][i + k - 1] == 1) && (mot_pos[n2][j + k - 1] == 1)) {
                                mot_match[k] = 1;
...
*/
                        ...
                        if (open_pos[n1][n2][i + k - 1]) {
                            bound_test = bound_test * (j + k - 1 >= lb_int[i + k - 1]);
                            bound_test = bound_test * (j + k - 1 <= ub_int[i + k - 1]);
                            ...
                            if (start_pep || (crick_strand && start_pep_c))
                                if ((wgt_type > 1) && ((k % 3) == 1)) {
                                    trpl_start = 1;

                                    trpl_start = trpl_start * (j + k >= lb_int[i + k]);
                                    trpl_start = trpl_start * (j + k <= ub_int[i + k]);
                                    trpl_start = trpl_start * open_pos[n1][n2][i + k];

                                    trpl_start = trpl_start * (j + k + 1 >= lb_int[i + k + 1]);
                                    trpl_start = trpl_start * (j + k + 1 <= ub_int[i + k + 1]);
                                    trpl_start = trpl_start * open_pos[n1][n2][i + k + 1];
                                }

                            if (bound_test && ((wgt_type != 2) || trpl_start)) {
                                if (start_pep)
                                    if ((wgt_type == 0) || ((wgt_type > 1) && trpl_start))
                                ...
                                if (start_dna)
                                    if (wgt_type % 2)
                                        if (!strict || (seq[n1][i + k - 1] == 'A' || seq[n1][i + k - 1] == 'C' || seq[n1][i + k - 1] == 'T' || seq[n1][i + k - 1] == 'G' || seq[n1][i + k - 1] == 'U'))
                                            match_d = match_d + (seq[n1][i + k - 1] == seq[n2][j + k - 1]);
                                ...
                                /*
   if( wgt_k_match > 0 )     printf(" k = %d min_dia = %d
   wgt_k_match = %f thr = %f \n", k, min_dia , wgt_k_match , thr
   );
*/
                                if (k >= min_dia)
                                    if (wgt_k_match > thr) {
                                        if ((current_dia = (struct pair_frag*)calloc(
                                                 1, sizeof(struct pair_frag)))
                                            == NULL) {
                                            printf("\n \n \n      ATTENTION: \n \n \n");
 ...
 */
    ...
    if (numsubseq) {
        ...
        if ((pair_dia = (struct multi_frag*)calloc(
                 (numsubseq + 1), sizeof(struct multi_frag)))
            == NULL) {
