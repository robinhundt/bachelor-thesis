...
extern int seq_read(char* in_file, char* sq[MAX_SEQNUM], char** sqn,
    char** fsqn);
...
extern void ali_arrange(int fragno, struct multi_frag* smp, FILE* fp, FILE* fp2,
    FILE* fp3, FILE* fp4, FILE* fp_csc);
...
main(int argc, char** argv)
{
    ...
    if (argc == 1) {
        ...
        printf("     -n    DNA sequences; similarity calculated at the nucleotide "
               "level \n\n");
        printf("     -nt   DNA sequences; similarity calculated at the peptide "
               "level\n");
        ...
        printf(
            "    DIALIGN USER_GUIDE that should come with the DIALIGN package.\n");
    ...
    if ((!strcmp(input_name + strlen(input_name) - 4, ".seq")) || (!strcmp(input_name + strlen(input_name) - 3, ".fa")) || (!strcmp(input_name + strlen(input_name) - 6, ".fasta")))
    ...
    if (seqnum < 2) {
        if (cd_gobics) {
            printf("\n\n         Something is wrong with your sequence file. Maybe "
                   "you entered a\n");
            printf("         MS WORD or RFT file or your file contains only one "
                   "single sequence.\n");
            printf("         Please note that our server only accepts plain text "
                   "files. \n\n");
            printf(
                "         For more information, please consult our online manual \n");
            ...
            printf("\n\n         Your sequence file containes only a single "
                   "sequence.\n");
            printf("         Please make sure your input file contains at least two "
                   "sequences.\n\n");
            printf(
                "         For more information, please consult the online manual \n");
            printf("         at the DIALIGN home page: \n\n");
            printf("             "
                   "http://bibiserv.techfak.uni-bielefeld.de/dialign/manual.html ");
        }
    ...
    if ((glob_sim = (float**)calloc(seqnum, sizeof(float*))) == NULL) {
        printf("Problems with memory allocation for glob_sim\n");
        exit(1);
    }

    for (i = 0; i < seqnum; i++) {
        if ((glob_sim[i] = (float*)calloc(seqnum, sizeof(float))) == NULL) {
            printf("Problems with memory allocation for glob_sim \n");
            exit(1);
        }
    }
...
\****************************/
    ...
    for (i = 0; i < seqnum; i++) {
        if ((open_pos[i] = (int**)calloc(seqnum, sizeof(int*))) == NULL) {
            printf("Problems with memory allocation for open_pos\n");
            exit(1);
        }
    }
    ...
    for (i = 0; i < seqnum; i++)
        for (j = 0; j < seqnum; j++) {
            if ((open_pos[i][j] = (int*)calloc((seqlen[i] + 2), sizeof(int))) == NULL) {
                printf("Problems with memory allocation for open_pos\n");
                exit(1);
            }
        }
...
*/
    ...
    if ((num_dia_bf = (int*)calloc((max_itnum + 1), sizeof(int))) == NULL) {
        printf(" problems with memory allocation for `num_dia_bf' !  \n \n");
        exit(1);
    }

    if ((num_dia_af = (int*)calloc((max_itnum + 1), sizeof(int))) == NULL) {
...
*/
                ...
                for (k = 0; k < num_dia_p; k++) {
                    *current_dia = pair_dia[k];
                    ...
                    current_dia->next = (struct multi_frag*)calloc(1, sizeof(struct multi_frag));
                    end_dia = current_dia;
                ...
                while (diagonal1->next != NULL) {
                    ...
                    if (print_status)
                        if ((dia_counter % 100) == 0) {
                            ...
                            fprintf(fp_st,
                                "      calculating overlap weight for diagonals\n");
