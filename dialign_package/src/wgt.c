...
void mem_alloc()
{
    ...
    if (wgt_type == 0) {
        ...
        if ((wgt_prot = (float**)calloc((max_dia + 1), sizeof(float*))) == NULL) {
    ...
    if (wgt_type % 2) {
        ...
        if ((wgt_dna = (float**)calloc((max_dia + 1), sizeof(float*))) == NULL) {
            printf(" problems with memory allocation for `weights' !  \n \n");
            exit(1);
        }
    }
    ...
    if (wgt_type > 1) {
        ...
        if ((wgt_trans = (float**)calloc((max_dia + 1), sizeof(float*))) == NULL) {
            printf(" problems with memory allocation for `weights' !  \n \n");
            exit(1);
        }
    }
    ...
    for (i = 1; i <= max_dia; i++) {
        if (wgt_type == 0) {
            if ((tp400_prot[i] = (double*)calloc(((i + 1) * max_sim_score),
                     sizeof(double)))
                == NULL) {
                printf(" problems with memory allocation for `tp400_prot' !  \n \n");
                exit(1);
            ...
            if ((wgt_prot[i] = (float*)calloc(((i + 1) * max_sim_score),
                     sizeof(float)))
                == NULL) {
                printf(" problems with memory allocation for `weights'!\n\n");
                exit(1);
        ...
        if (wgt_type % 2) {
            if ((tp400_dna[i] = (double*)calloc(((i + 1)), sizeof(double))) == NULL) {
                printf(" problems with memory allocation for `tp400_dna' !  \n \n");
                exit(1);
            }

            if ((wgt_dna[i] = (float*)calloc(((i + 1)), sizeof(float))) == NULL) {
                printf(" problems with memory allocation for `weights'!\n\n");
                exit(1);
            }
        }
        ...
        if (wgt_type > 1) {
            if ((tp400_trans[i] = (double*)calloc(((i + 1) * max_sim_score),
                     sizeof(double)))
                == NULL) {
            ...
            if ((wgt_trans[i] = (float*)calloc(((i + 1) * max_sim_score),
                     sizeof(float)))
                == NULL) {
                printf(" problems with memory allocation for `weights'!\n\n");
                exit(1);
