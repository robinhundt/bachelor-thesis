...
void ali_arrange(int fragno, struct multi_frag* d, FILE* fp, FILE* fp2, FILE* fp3,
    FILE* fp4, FILE* fp_col_score)
{
    ...
    if (fragno >= 0) {
        ...
        if ((weight_count = (float*)calloc((endlen + 2), sizeof(float))) == NULL) {
        ...
        if ((plus_count = (int*)calloc((endlen + 2), sizeof(int))) == NULL) {
            printf(" problems with memory allocation for `plus_count' !\n \n");
            exit(1);
        }

        if ((minus_count = (int*)calloc((endlen + 2), sizeof(int))) == NULL) {
            printf(" problems with memory allocation for `minus_count' !\n \n");
            exit(1);
        }

        if ((nuc_count = (int*)calloc((endlen + 2), sizeof(int))) == NULL) {
            printf(" problems with memory allocation for `nuc_count' !\n \n");
            exit(1);
        }

        if ((frg_involved = (int*)calloc((endlen + 2), sizeof(int))) == NULL) {
...
*/
        ...
        for (hv = 0; hv < fragno; hv++)
            for (k = 0; k < 2; k++)
                for (i = fragments[hv].b[k]; i < fragments[hv].b[k] + fragments[hv].ext;
                     i++)
        ...
        for (p = 0; p < endlen; p++) {
            s1 = 0;
            while ((endseq[s1][p] == tolower(endseq[s1][p])) && (s1 < (seqnum - 1)) /* no capital letter */
                )
                s1++;
            ...
            if (s1 < (seqnum - 1)) {
                for (s2 = s1 + 1; s2 < seqnum; s2++) {
                    ...
                    /* endseq[s2][p] capital letter */
                    {
                        ...
                        if (!aligned) /* i.e.endseq[s1][p] not aligned with end seq[s2][p]*/
        ...
        plot_calc(num_all_it_dia, endlen, weight_count, plot, all_it_dia,
            fp_col_score);

        wgt_type_count(num_all_it_dia, endlen, plus_count, minus_count, nuc_count,
            frg_involved, all_it_dia);
...
void para_print(char* s_f, FILE* fpi)
{
...

    {
        if (cd_gobics) {
            ...
            fprintf(fpi, "          Burkhard Morgenstern, Said Abdeddaim and Michael "
                         "Brudno \n\n");
   ...
   p_count++);
      ...
      if( lmax != MAX_DIA )
        {
          fprintf(fpi,"    %2d) maximum length of fragments = %d", p_count++ ,
   lmax );
          if( wgt_type == 0)
