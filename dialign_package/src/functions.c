...
int translate(char c1, char c2, char c3, int seqno, int pos)
{
    ...
    if ((c1 != 'A' && c1 != 'T' && c1 != 'G' && c1 != 'C') || (c2 != 'A' && c2 != 'T' && c2 != 'G' && c2 != 'C') || (c3 != 'A' && c3 != 'T' && c3 != 'G' && c3 != 'C'))
        return (0);
    else
        return (amac);
...
void change_first(struct multi_frag* a, struct multi_frag* b)
{
    ...
    if (a == b) { /* Change the first list-element with the second one (old
                 first-el.). */
        c = *a;
        an = a->next;
        bn = a->next->next;
    ...
    } else /* Change the new first list-el. with the old first-el. */
    {
        ...
        *a = *(b->next); /* Whrite the value of the old first-el. on the place of
                    the new first-el.*/
        a->next = bn; /* Bend his "next" pointer at the next el. of the old first-el. */

        *(b->next) = c; /* Whrite the value of the new fist-el. on the place of the old
first-el. */
...
void quicksort_ow(struct multi_frag** array, int left, int right)
{
    ...
    if (left < r)
        quicksort_ow(array, left, r);
    if (l < right)
        quicksort_ow(array, l, right);
...
void quicksort_weight(struct multi_frag** array, int left, int right)
{
    ...
    if (left < r)
        quicksort_weight(array, left, r);
    if (l < right)
        quicksort_weight(array, l, right);
   ...
   have any overlap */
    ...
    for (i = 0; i < 2; i++)
        for (j = 0; j < 2; j++)
            if (sm1->s[i] == sm2->s[j])
                if (sm1->s[j] != sm2->s[i])
                    if (sm1->b[i] < sm2->b[j] + sm2->ext && sm2->b[j] < sm1->b[i] + sm1->ext) {
                        conslen = mini2(sm1->b[i] + sm1->ext, sm2->b[j] + sm2->ext) - maxi2(sm1->b[i], sm2->b[j]);
                        if ((trans == 0) || ((conslen % 3) == 0)) {
                            s1 = sm1->s[(i + 1) % 2];
                            s2 = sm2->s[(j + 1) % 2];
                            ...
                            for (k = 0; k < conslen; k++) {
                                if ((wgt_type == 0) || (trans && ((k % 3) == 0)))
                                    match = match + sim_score[amino[s1][b1 + k]][amino[s2][b2 + k]];
                                else
   ...
   in clos (when GABIOS is used) are changed accordingly */
    ...
    for (nv = 0; nv < number_bf; nv++) {
             ...
             already included into the alignment */
        {
            ...
            dia->sel = 1;
            glob_sim[as[0]][as[1]] = glob_sim[as[0]][as[1]] + dia->weight;
        ...
        if ((istep == 0) && anchors && (seqnum > 2)) {
            fprintf(fp_cap, " anchor %d %d %d %d %d %f ", as[0] + 1, as[1] + 1, ab[0],
                ab[1], aext, awgt);
            if (dia->sel == 0)
...
wgt_type_count(int num, int e_len, int* plus_cnt, int* minus_cnt, int* nuc_cnt,
    int* frg_inv, struct multi_frag* dia)
{
    int i, dc, pc, s1, pos;
...
plot_calc(int num, int e_len, float* w_count, float* pl, struct multi_frag* dia,
    FILE* fp_csc)
{
    int i, dc, pc, s1, pos;
...
void av_tree_print()
{
    ...
    if ((all_clades = (struct subtree*)calloc(seqnum, sizeof(struct subtree))) == NULL) {
        printf(" problems with memory allocation for `all_clades'\n \n");
        exit(1);
    }

    if ((clade_similarity = (double**)calloc(seqnum, sizeof(double*))) == NULL)
        exit(1);

    for (i = 0; i < seqnum; i++)
        if ((clade_similarity[i] = (double*)calloc(seqnum, sizeof(double))) == NULL)
            exit(1);

    if ((string = (char*)calloc(seqnum * 100, sizeof(char))) == NULL) {
        printf(" problems with memory allocation for `string'\n \n");
        exit(1);
    }

    for (i = 0; i < seqnum; i++) {
        if ((all_clades[i].member = (int*)calloc(seqnum, sizeof(int))) == NULL) {
            printf(" problems with memory allocation for `all_clades'\n \n");
            exit(1);
        }

        if ((all_clades[i].name = (char*)calloc(seqnum * 100, sizeof(char))) == NULL) {
            printf(" problems with memory allocation for `all_clades'\n \n");
            exit(1);
        }
    ...
    for (connect = 1; connect < seqnum; connect++) {
...

        {
                    ...
                    if (i != m1)
                        if (i != m2) {
                            if (!strcmp(clust_sim, "av"))
                                new_similarity = (clade_similarity[i][m1] * all_clades[m1].member_num + clade_similarity[i][m2] * all_clades[m2].member_num) / (all_clades[m1].member_num + all_clades[m2].member_num);

                            if (!strcmp(clust_sim, "max"))
                                new_similarity = maxf2(clade_similarity[i][m1], clade_similarity[i][m2]);

                            if (!strcmp(clust_sim, "min"))
                                new_similarity = minf2(clade_similarity[i][m1], clade_similarity[i][m2]);
            ...
            for (k = 0; k < all_clades[m2].member_num; k++)
                all_clades[m1].member[all_clades[m1].member_num + k] = all_clades[m2].member[k];

            all_clades[m1].member_num = all_clades[m1].member_num + all_clades[m2].member_num;
            ...
            while (diagonal != NULL) {
                ...
                if (diagonal->s[0] == i && diagonal->s[1] == j) {
                    ...
                    if (long_output) {
                        ...
                        if (wgt_type == 2 || ((wgt_type == 3) && diagonal->trans))
                            this_frag_trans = 1;
                        else
                            this_frag_trans = 0;
